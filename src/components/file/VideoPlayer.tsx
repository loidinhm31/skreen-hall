    ...
    >
      {videos.map((video, index) => (
        <div
          ...
          style={{
            background:
              index === currentVideoIndex ? "linear-gradient(45deg, #667eea, #764ba2)" : "rgba(255, 255, 255, 0.1)",
            margin: "8px 0",
            padding: "12px 15px",
            borderRadius: "6px",
            cursor: "pointer",
            ...
            alignItems: "center",
            gap: "10px",
            boxShadow: index === currentVideoIndex ? "0 4px 15px rgba(102, 126, 234, 0.3)" : "none",
        ...
        >
          ...
          <span style={{ flex: 1, fontWeight: "500" }}>{sanitizeFileName(video.name)}</span>
          <span style={{ opacity: 0.7, fontSize: "12px" }}>{formatFileSize(video.size)}</span>
        </div>
      ))}
    </div>
  );
